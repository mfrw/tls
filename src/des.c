...
static const int pc2_table[] = {14, 17, 11, 24, 1,  5,	3,  28, 15, 6,	21, 10,
...
static void ror(unsigned char *target) {
	...
	target[3] = (((target[3] >> 1) | ((target[2] & 0x01) << 7)) & ~0x08) |
		    carry_right;
...
static void des_block_operate(const unsigned char plaintext[DES_BLOCK_SIZE],
	...
	for (round = 0; round < 16; round++) {
		// “Feistel function” on the first half of the block in
		// ‘ip_block’ “Expansion”. This permutation only looks at the
		// first four bytes (32 bits of ip_block); 16 of these are
		// repeated  in “expansion_table”.
		permute(expansion_block, ip_block + 4, expansion_table, 6);
		// “Key mixing” rotate both halves of the initial key
		...
		// Substitution; “copy” from updated expansion block to
		// ciphertext block
		...
		substitution_block[3] =
		    sbox[6][(expansion_block[4] & 0x0F) << 2 |
			    (expansion_block[5] & 0xC0) >> 6]
		    << 4;
		substitution_block[3] |= sbox[7][(expansion_block[5] & 0x3F)];
		// Permutation
		permute(pbox_target, substitution_block, p_table,
			DES_BLOCK_SIZE / 2);
		// Recombination. XOR the pbox with left half and then switch
		// sides.
		memcpy((void *)recomb_box, (void *)ip_block,
		       DES_BLOCK_SIZE / 2);
